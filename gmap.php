...
if (!class_exists('gmap_main')):
    ...
    class gmap_main {
        ...
        public function gmap_create_menu() {
            ...
            $settings_page = add_menu_page($this->plugin_name.' Settings', $this->plugin_name, 'administrator', 'gmap-settings', array($this, 'gmap_settings_page'), 'dashicons-admin-generic');
            
            // add submenu under Google Map
            $help_page = add_submenu_page('gmap-settings', $this->plugin_name . ' Help', $this->plugin_name . ' Help', 'administrator', 'gmap-help', array($this, 'gmap_help_page'));
        ...
        public function admin_notice_ignore(){
            if(isset($_GET["gmap_admin_notice_ignore"]) && $_GET["gmap_admin_notice_ignore"] == 1){               
                update_option('gmap_admin_notice_ignore', 1);
                $query_str = remove_query_arg('gmap_admin_notice_ignore');
                wp_redirect($query_str);
                exit;           
            }
        }
        public function wd_admin_notices(){
            if(get_option('gmap_admin_notice_ignore') != 1 && get_option("gmwd_version") == false){
                $query_str = add_query_arg(array('gmap_admin_notice_ignore'=> 1));
        ...
        ?>
                <div class="update-nag wd-admin-notice">
                    <div class="gmwd-notice-logo"></div>
                    <div class="gmwd-notice-text">
                        <p class="wd-notice-title">Google Map is not supported anymore.</p>
                        <p class="wd-notice-body">Please download and start using feature rich <a href="https://wordpress.org/plugins/wd-google-maps/" target="_blank">Google Maps WD</a> plugin instead.</p>
                    </div>
                    <div class="gmwd-notice-download">
                        <a href="<?php echo $install_url; ?>">Install Now</a>
                    </div>
                    <a href="<?php echo $query_str ;?>" class="dashicons dashicons-dismiss"></a>
                </div> 

        <?php
                wp_enqueue_style('gmap_notices',  plugins_url(plugin_basename(dirname(__FILE__))) . '/assets/css/notices.css', array());
            }
        }
